class Sanction
  attr_accessor :alert_id, :start_date

  def initialize(api_data)
    @alert_id = api_data[:alert_id]
  ...
  def description
    SANCTIONS[alert_id][:description] if SANCTIONS[alert_id]
  end

  def title
    SANCTIONS[alert_id][:title] if SANCTIONS[alert_id]
  end

  def guilty_but_not_prohibited?
    alert_id == "7924fe90-483c-49f8-84fc-674feddba848	"
  end
end
