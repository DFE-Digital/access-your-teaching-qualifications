...
- **GitHub** — you need write access to the [access-your-teaching-qualifications](https://github.com/DFE-Digital/access-your-teaching-qualifications) repository to trigger workflows, and admin access to update branch protection rules for the pipeline freeze.
...
It is important to note that AYTQ (Access Your Teaching Qualifications) and CTR (Check a Teacher's Record) share this repository and their deployments, taking down one service will also take down the other. Therefore, during a disaster recovery scenario for either service, both services will be affected and restoration efforts will impact both.
...
- `kubectl -n tra-test scale deployment access-your-teaching-qualifications-test --replicas 0`
- `kubectl -n tra-test scale deployment access-your-teaching-qualifications-test-worker --replicas 0`
...
Run the [Recover deleted postgres database server workflow](https://github.com/DFE-Digital/access-your-teaching-qualifications/actions/workflows/postgres-recover-deleted-db.yml) to recreate the missing postgres database.
...
|----------------|--------------------|
| production     | s189p01-aytq-pd-pg |
| preproduction  | s189t01-aytq-pp-pg |
| test           | s189t01-aytq-ts-pg |
...
Run the [deploy workflow](https://github.com/DFE-Digital/access-your-teaching-qualifications/actions/workflows/build-and-deploy.yml) to recreate the missing postgres database as detailed below. This will create an empty database for us to restore the data into in the next step.
...
|----------------|-----------------------------------------|--------------------|
| production     | s189-teacher-services-cloud-production  | s189p01-aytq-pd-rg |
| preproduction  | s189-teacher-services-cloud-test        | s189t01-aytq-pp-rg |
| test           | s189-teacher-services-cloud-test        | s189t01-aytq-ts-rg |

##### Restore the data from previous backup in Azure storage

Run the [Restore database from Azure storage workflow](https://github.com/DFE-Digital/access-your-teaching-qualifications/actions/workflows/database-restore.yml). This will restore the most recent backup taken by the scheduled github workflow, so there may be some data loss depending on when the last backup was taken.
...
If you need to find the filename of a nightly backup, you can get it from the output of the [scheduled backup GitHub Action](https://github.com/DFE-Digital/access-your-teaching-qualifications/actions/workflows/database-backup.yml) workflow run that created it.
...
- `kubectl -n tra-test get deployments`
- `kubectl -n tra-test scale deployment access-your-teaching-qualifications-test --replicas 2`
- `kubectl -n tra-test scale deployment access-your-teaching-qualifications-test-worker --replicas 1`
...
| Service |  Environment   | Healthcheck URL                                                                 |
|---------|----------------|---------------------------------------------------------------------------------|
| AYTQ    |  production    | https://access-your-teaching-qualifications.education.gov.uk/health/all         |
| AYTQ    |  preproduction | https://preprod.access-your-teaching-qualifications.education.gov.uk/health/all |
| AYTQ    |  test          | https://test.access-your-teaching-qualifications.education.gov.uk/health/all    |
| CTR     |  production    | https://check-a-teachers-record.education.gov.uk/health/all                     |
| CTR     |  preproduction | https://preprod.check-a-teachers-record.education.gov.uk/health/all             |
| CTR     |  test          | https://test.check-a-teachers-record.education.gov.uk/health/all                |
...
1. [Start the incident process](#start-the-incident-process-if-not-already-in-progress)
1. [Freeze pipeline](#freeze-pipeline)
1. [Back up the database (optional)](#back-up-the-database-optional)
1. [Restore postgres database](#restore-postgres-database)
...
Use the [Backup database to Azure storage workflow](https://github.com/DFE-Digital/access-your-teaching-qualifications/actions/workflows/database-backup.yml) to save a copy of the flawed database. Use a specific name to identify the backup file later on.
...
e.g. `bin/konduit.sh -x -n tra-test -s <name-of-ptr-server> access-your-teaching-qualifications-test -- psql`

To connect to the existing live postgres server for comparison:
- Run: `bin/konduit.sh -x name-of-deployment -- psql`

e.g. `bin/konduit.sh -x access-your-teaching-qualifications-test -- psql`
...
To do this use the [Backup database to Azure storage](https://github.com/DFE-Digital/access-your-teaching-qualifications/actions/workflows/database-backup.yml) workflow and choose the restored server as input. Use a specific name to identify the backup file later on.
